...
def main(args):
    ...
    if args.task == "all":
        task_filenames = glob(f"{args.screenspot_test}/annotations/*.json")
    else:
    ...
    for dataset in task_filenames:
            ...
            for gt_type in gt_types:
                for lang in languages:
                    for task_instance in task_data:
                        ...
                        task_instance["task_filename"] = dataset
        ...
        print(f"Num of sample in {dataset}: {len(task_data)} * {len(inst_styles)} * {len(gt_types)} * {len(languages)} = {len(task_data) * len(inst_styles) * len(gt_types) * len(languages)}")
