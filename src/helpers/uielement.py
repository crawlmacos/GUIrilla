...
class UIElement:
    ...
    def __init__(self, element, offset_x=0, offset_y=0, parent = None, parents_visible_bbox=None):
        # set attributes
        ...
        if self.name is not None:
            ...
            self.name = "empty-name-" + self.role
                ...
                if (
                    ...
                    if children_elements is not None and len(children_elements) > 0:
                        found_children = UIElement.children(
                            children[0], offset_x, offset_y, parent = self, visible_bbox = self.visible_bbox
                        )
                        children_all = found_children
                    else:
                        children_all = UIElement.children(element, offset_x, offset_y, parent = self, visible_bbox = self.visible_bbox)
                else:
                    children_all = UIElement.children(element, offset_x, offset_y, parent = self, visible_bbox = self.visible_bbox)
            else:
                children_all = UIElement.children(element, offset_x, offset_y, parent = self, visible_bbox = self.visible_bbox)
    ...
    def content_hash(self, element):
        ...
        # this should possibly be removed as name would never be None
        name = ""
    ...
    # parse children
    @classmethod
    def children(cls, element, offset_x=0, offset_y=0, parent = None, visible_bbox=None):
        children = element_attribute(element, ApplicationServices.kAXChildrenAttribute)
        ...
        for child in found_children:
            child = cls(child, offset_x, offset_y, parent, visible_bbox)
            result.append(child)
        return result
